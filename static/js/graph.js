queue()
    .defer(d3.csv, "data/Salaries.csv ") // 2 arguments, format of data (1) and path to data (2)
    .await(makeGraphs);
    
function makeGraphs(error, salaryData) { // 2 arugments, 1st var is error, 2nd is var that data will be passed into queue js 
...
var percentageThatAreProf = ndx.groupAll().reduce(
    function (p, v) {
        if (v.sex === gender) { //new argument from ndx 
             p.count++;
                if (v.rank === "Prof") {
                    p.are_prof++;
                }
    ...
    dc.numberDisplay(element) // formerly "#percentage-of-women-professors"
...
function show_gender_balance(ndx) {
    ...
    var group = dim.group(); // no reduce as we just count the rows
...
function show_service_to_salary_correlation(ndx) {
    ...
    var genderColors = d3.scale.ordinal()
        .domain(["Female", "Male"])
        .range(["pink", "blue"]) // domain and range need to have same amounts as the correlate 
